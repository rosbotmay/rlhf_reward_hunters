...
class RewardModel(nn.Module):
    def __init__(self, num_features, num_actions, hidden_size=64):
        super(RewardModel, self).__init__()
        self.net = nn.Sequential(
            nn.Linear(num_features * 2 + num_actions, hidden_size),  # Include next state
            nn.ReLU(),
...
class PPO:
    ...
    def compute_gae(self, rewards, values, next_value, dones):
        advantages = []
        gae = 0
        values = values + [next_value]  # Include the bootstrap value
            ...
            for p in batch:
                try:
                    ...
                    for s, a, _ in p['tau1']:
                        ...
                        tau1_sum += agent.reward_model(s_t, a_t, s_t).squeeze()
                    for s, a, _ in p['tau2']:
                        s_t = torch.FloatTensor(s).unsqueeze(0).to(DEVICE)
                        a_t = action_to_tensor(a, agent.action_space, action_size, DEVICE).unsqueeze(0)
                        tau2_sum += agent.reward_model(s_t, a_t, s_t).squeeze()
