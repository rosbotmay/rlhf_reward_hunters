...
from rlhf_reward_hunters.src.DPO.dpo import train_dpo, evaluate_dpo_policy
import os

if __name__ == "__main__":
    # initialize & load pretrained policy
    policy = Policy(state_size=4, action_size=2).to(DEVICE)
    policy.load_state_dict(torch.load(f"{DATA_DIR}/policy_pi2.pth", map_location=torch.device('cpu')))
    
    # fine-tune via DPO
    train_dpo(policy, prefs_file ="prefs_expert_vs_subexpert.pkl")

    ckpt = os.path.join(CHECKPOINT_DIR, "dpo_policy_expert_sub_700.pth")
