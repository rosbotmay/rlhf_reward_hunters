...
from config import DATA_DIR,ENV_NAME
...
if __name__ == "__main__":
    ...
    policy_pi1 = Policy(state_size=obs_dim, action_size=act_dim).to(DEVICE)

    optimizer_pi1 = optim.Adam(policy_pi1.parameters(), lr=1e-3)
    scores_pi1 = reinforce_rwd2go_baseline(env, policy_pi1, optimizer_pi1, early_stop=True, n_episodes=2000, max_reward = -110)
   ...
   # Generate subexpert's policy, pi2
    ...
    policy_pi2 = Policy(state_size=obs_dim, action_size=act_dim).to(DEVICE)

    optimizer_pi2 = optim.Adam(policy_pi2.parameters(), lr=1e-3)
    scores_pi2 = reinforce_rwd2go_baseline(env, policy_pi2, optimizer_pi2, early_stop=True, n_episodes=2000, max_reward = -140)
    ...
    ax.set_ylabel('Total reward (MountainCar)', fontsize=20)
